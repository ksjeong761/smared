...
extern "C" {
    ...
    Java_kr_ac_kpu_block_smared_OCRImagePreprocessActivity_imageprocessing (
        ...
        // 흑백화 -> 블러링 -> 이진화 -> 닫기 연산 -> 블러링
        cvtColor(img_input, img_output, CV_BGR2GRAY);
        GaussianBlur(img_output, img_output, Size(7,7), 1.5, 1.5);
        adaptiveThreshold(img_output, img_output, 255, CV_ADAPTIVE_THRESH_MEAN_C, CV_THRESH_BINARY, 31, 5);
        morphologyEx(img_output, img_output, MORPH_CLOSE, element5);
        GaussianBlur(img_output, img_output, Size(7,7), 1.5, 1.5);
     }
}

// 인식률을 높이기 위해 시도했던 흔적들
//Mat imgCanny;
